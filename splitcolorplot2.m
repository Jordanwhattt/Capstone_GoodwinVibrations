%This file was downloaded oneline but modified to make larger lines

function split = splitcolorplot2(x,y,val,fmt1,fmt2)
...
%  format options)
%  HACK! to deal with variable number of inputs, without using varargin,
%  which is uninformative as a function signature
switch nargin
    case 0
        error('At least one input is required')
    case 1
        [x,y,val,clr,mrkr,lntp,lngth] = checkinputs(x);
    case 2
        [x,y,val,clr,mrkr,lntp,lngth] = checkinputs(x,y);
    case 3
        [x,y,val,clr,mrkr,lntp,lngth] = checkinputs(x,y,val);
    case 4
        [x,y,val,clr,mrkr,lntp,lngth] = checkinputs(x,y,val,fmt1);
    case 5
        [x,y,val,clr,mrkr,lntp,lngth] = checkinputs(x,y,val,fmt1,fmt2);
    otherwise
...
for k=2:nidx
    ...
    fmt = (y(k1)<=val) + 1;
    %  Plot line segment
    line(x(k1:k2),y(k1:k2),'color',clr{fmt},'marker',mrkr{fmt},'linestyle',lntp{fmt},'LineWidth', 2)
    ...
    if k<nidx
        ...
        line([x1,x0],[y1,val],'color',clr{fmt},'linestyle',lntp{fmt},'LineWidth', 2)
        %  Change formats and plot from y = val to start of 2nd segment
        fmt = mod(fmt,2) + 1;
        line([x0,x2],[val,y2],'color',clr{fmt},'linestyle',lntp{fmt},'LineWidth', 2)
    end
end
end

%  Parse all the inputs
function [x,y,val,clr,mrkr,lntp,lngth] = checkinputs(x,y,val,fmt1,fmt2)
