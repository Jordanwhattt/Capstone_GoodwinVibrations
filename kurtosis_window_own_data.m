...
wind_list = zeros(1,2);
for j = 5:kurt_len
    if (kurt_bin(j-1)+kurt_bin(j-2)+kurt_bin(j-3)+kurt_bin(j-4)) == 0
        if kurt_bin(j) == 1
            wind_start = time_index(1,j-4);
            k = j;
            while (kurt_bin(k+1)+kurt_bin(k+2)+kurt_bin(k+3)+kurt_bin(k+4)) ~= 0
                k = k+1;
            end
            wind_end = time_index(1,j+4);
