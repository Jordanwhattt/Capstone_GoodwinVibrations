...
for k=2:nidx
    ...
    fmt = (y(k1)<=val) + 1;
    %  Plot line segment
    line(x(k1:k2),y(k1:k2),'color',clr{fmt},'marker',mrkr{fmt},'linestyle',lntp{fmt},'LineWidth', 1)
    ...
    if k<nidx
        ...
        line([x1,x0],[y1,val],'color',clr{fmt},'linestyle',lntp{fmt},'LineWidth', 1)
        %  Change formats and plot from y = val to start of 2nd segment
        fmt = mod(fmt,2) + 1;
        line([x0,x2],[val,y2],'color',clr{fmt},'linestyle',lntp{fmt},'LineWidth', 1)
    end
end
end
